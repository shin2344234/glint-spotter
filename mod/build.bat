@echo off
rem Glint Spotter build: MSVC Build Tools 2022 + the CMake and Ninja they bundle.
rem   build.bat          configure (first run) and build Release into build\, stage dist\
rem   build.bat clean    wipe build\ first
rem Call it by full quoted path from PowerShell; the space in the repo path breaks
rem a bare "cmd /c build.bat".
setlocal
