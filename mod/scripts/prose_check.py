"""Check prose for AI hallmarks before it goes out as Seth's words.
    ...
    py -3 mod/scripts/prose_check.py private/nexus/nexus-post-1.1.22.bbcode
    py -3 mod/scripts/prose_check.py --all

Ported from Master Looter, where it was written and where its lists grew. Keep
the two in step: a tic caught in either project is a tic in both, so add it
there as well rather than letting the copies drift.

Pass every document of one release in a single call. The repeated-sentence
check only fires across files given together, and the post, the changelog and
the Discord announcement are read by the same people within a minute of each
other.
...
PROPER = [
    "Definitive Mod Manager", "Crimson Desert", "Glint Spotter", "Master Looter",
    "Crimson Route", "Flight Freedom", "Skip All Loading Scene",
    "Ultimate ASI Loader", "Nexus Mods", "Vortex", "Mod Organizer",
    "Blinding Flash", "Remove Marker", "Steam Input", "DS4Windows",
    "DualSense", "DualShock", "Windows", "Steam", "GitHub", "Discord",
    "VirusTotal", "Microsoft", "Symantec", "Cynet", "LimeWire",
    "Kliff", "Damiane", "Oongka",
]
...
#
#   nexus-release.md carries the description and the changelog copy in full, so
#   every sentence in it is a duplicate by construction.
#
#   README.md and the mod page description are long-lived reference documents
#   about one mod. They are
...
def main():
    ...
    if "--all" in sys.argv or not args:
        ...
        args = []
        for sub in ("nexus", "discord"):
            d = os.path.join(root, "private", sub)
            if not os.path.isdir(d):
                continue
            args += [os.path.join(d, f) for f in sorted(os.listdir(d))
                     if f.endswith((".txt", ".bbcode", ".md"))
